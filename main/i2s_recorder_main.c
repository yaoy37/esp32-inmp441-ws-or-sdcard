...
void mount_sdcard(void)
{
    ...
    ESP_LOGI(TAG, "Initializing SD card end");
...
void record_wav(uint32_t rec_time)
{
    ...
    char name[26] = "abcdefghijklmnopqrstuvwxyz";
    ...
    while (fragment < 10)
    {
        ...
        char file_name[30];
        sprintf(file_name, "%s/record_%c.wav", SD_MOUNT_POINT, name[fragment]);
        printf("file:%s\n", file_name);
        struct stat st;
        ...
        while (flash_wr_size < flash_rec_time)
        {
            ...
            i2s_read(CONFIG_EXAMPLE_I2S_CH, i2s_buff, sizeof(i2s_buff), &bytes_read, 100);
            // Write the samples to the WAV file
            // printf("%ls\n",i2s_buff);
            fwrite(i2s_buff, 1, sizeof(i2s_buff), f);
...
void read_sound_task(void)
{
    while (1)
    {
        ...
        if (esp_websocket_client_is_connected(websocket_client))
        {
            ...
            // esp_websocket_client_send_text(websocket_client, data, strlen(data), portMAX_DELAY);
            i2s_read(CONFIG_EXAMPLE_I2S_CH, (char *)i2s_buff, sizeof(i2s_buff), &bytes_read, 5000);

            esp_websocket_client_send_bin(websocket_client, (const char *)i2s_buff, sizeof(i2s_buff), 5000);
            // check_memory_usage();
            // vTaskDelay(1000 / portTICK_RATE_MS);
        }
        else
        {
            ESP_LOGW(TAG, "WebSocket not connected, reconnecting...123");
            if (websocket_client != NULL)
            {
